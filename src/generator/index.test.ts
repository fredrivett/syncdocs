...
vi.mock("./ai-client.js", () => {
	return {
		AIClient: class MockAIClient {
			generateDoc = vi
				.fn()
				.mockResolvedValue("# Test Function\n\nThis is a test function.");
		},
	};
});
	...
	describe("generate", () => {
		it("should generate documentation for a symbol", async () => {
			const symbol: SymbolInfo = {
				...
				body: "{ return a + b }",
				fullText:
					"function testFunction(a: number, b: number) { return a + b }",
				startLine: 1,
				endLine: 3,
			};
		...
		it("should include related symbols as dependencies", async () => {
			const mainSymbol: SymbolInfo = {
				...
				fullText:
					"function processData(data: Data[]) { return data.map(transform) }",
				startLine: 1,
				endLine: 3,
			};
			...
			const relatedSymbol: SymbolInfo = {
				...
				fullText:
					"function transform(d: Data) { return { ...d, processed: true } }",
				startLine: 5,
				endLine: 7,
			};
